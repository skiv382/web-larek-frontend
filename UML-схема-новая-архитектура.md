...
```
src/architecture/commands/
├── Command.ts                    # Базовый класс команд
├── UserCommands.ts              # Экспорт всех команд пользователя
└── user-commands/               # Отдельные файлы команд
    ├── index.ts                 # Индексный файл экспортов
    ├── AddToBasketCommand.ts    # Команда добавления в корзину
    ├── RemoveFromBasketCommand.ts # Команда удаления из корзины
    ├── ClearBasketCommand.ts    # Команда очистки корзины
    ├── UpdateOrderFieldCommand.ts # Команда обновления полей заказа
    ├── OpenModalCommand.ts      # Команда открытия модального окна
    ├── CloseModalCommand.ts     # Команда закрытия модального окна
    └── LoadCatalogCommand.ts    # Команда загрузки каталога
```
...
- Модульная организация команд (по одному классу на файл)
